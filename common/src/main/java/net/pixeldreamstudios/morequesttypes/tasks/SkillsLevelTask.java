...
public class SkillsLevelTask extends Task {
    ...
    public void fillConfigGroup(ConfigGroup config) {
        ...
        config.addInt("required_level", requiredLevel, v -> requiredLevel = Math.max(1, v), 10, 1, 100000)
                .setNameKey("morequesttypes.task.skills_level.required");
    ...
    public void addMouseOverText(TooltipList list, TeamData teamData) {
        if (mode == Mode.TOTAL_LEVEL) {
            list.add(Component.translatable("morequesttypes.task.skills_level.tooltip.total", requiredLevel));
        } else {
            var shown = categoryId.isEmpty() ? "?" : categoryId;
            list.add(Component.translatable("morequesttypes.task.skills_level.tooltip.category", shown, requiredLevel));
        }
    }
